...
public interface JobPostingRepository extends JpaRepository<JobPosting, UUID> {
    ...
    List<JobPosting> findByVacancyIdOrderByCreatedAtDesc(UUID vacancyId);
    ...
    @Query("""
            ...
            """)
    List<JobPosting> findLiveByChannel(@Param("channel") JobPosting.Channel channel,
                                       @Param("today") LocalDate today);

    /** M278 — daily expiry sweep: PUBLISHED postings past their deadline. */
    @Query("""
            select p from JobPosting p
            where p.status = az.millers.hcm.recruitment.domain.JobPosting.Status.PUBLISHED
